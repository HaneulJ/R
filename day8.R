...
title2 = html_nodes(read_html('http://www.w3.org/Protocols/rfc2616/rfc2616.html'), 'div.toc h2') # CSS 선택자
title2 = html_nodes(read_html('http://www.w3.org/Protocols/rfc2616/rfc2616.html'), 'body > div > h2')
title2 = html_nodes(read_html('http://www.w3.org/Protocols/rfc2616/rfc2616.html'), '//div/h2')   # xpath(= /html/body/div/h2)
...
length(link)
link
article.href = unique(html_attr(link, 'href')) # html_attr(link, 'href'): href 속성 꺼내기
article.href
...
for(i in 1:length(img.src)){
  res = GET(paste('http://unico2013.dothome.co.kr/',img.src[i], sep="")) # 주소+이미지파일이름 => 화면에 하나씩 뜸
  writeBin(content(res, 'raw'), paste('c:/Temp/', img.src[i], sep="")) # 바이너리 형식 (저장장소, 형식 지정)
} # content(get함수 리턴값, 타입) 
  # raw: 응답된 형식 그대로
...
query <- URLencode(iconv("봄","euc-kr","UTF-8"));URLencode('ABC');URLencode('가나다')
...
bus_No <- "146" # 라우터 아이디? 노선정보조회 서비스에서 알아내야 함
# url 문자열 만들기
...
busRouteId <- df[df$busRouteNm == 146, "busRouteId"] # 중복제거(버스의 고유번호로)
busRouteId
...
response <- GET(url)
json_data <- content(response, type = 'text', encoding = "UTF-8") # 읽을 수 있는content구조로 만들기
json_obj <- fromJSON(json_data)
class(json_obj) # 리스트(R은 리스트로 객체 만듦)
df <- data.frame(json_obj)
df <- df[-1]
names(df) <- c("className", "unitName", "cycle", "keystatName", "dataValue")
View(df)
options(scipen=100) # 지수형태로 변환되어 있던 것들을 숫자 나열 형태로 설정정
View(df)
