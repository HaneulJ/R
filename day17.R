...
mtcars.lm <- lm(mpg ~ ., data=mtcars) # 남아있는 모든 변수를 독립변수로 사용: ~
summary(mtcars.lm)
...
AIC(mtcars.lm1, mtcars.lm2, mtcars.lm3)
...
summary(multico.lm) # 회귀분석을 했으니 유의미하지X

cor(multico[-1])

library(car)
vif(multico.lm)  # 분산팽창계수? 독립변수 간의 강한 상관계수

multico.lm <- lm(Y~X1+X2+X3, data=multico)
summary(multico.lm)
vif(multico.lm)

# 변수들간의 어떤 조합이 유의미한 결과를 만들어낼지 체크
multico.regsubsets <- regsubsets(x=Y ~ . - ID, data=multico, nbest=2) # ID 제외
...
print(mk)
cen <- c(mk$lon, mk$lat) # 센터 정보는 벡터로
...
ggmap(map) + 
  ...
  geom_text(aes(x=mk$lon, y=mk$lat, label="우리가 공부하는 곳", 
                color="red", vjust=1, hjust=1, family='maple'))
...
df <- data.frame(name=names,
                 lon=gc$lon,
                 lat=gc$lat)
cen <- c(mean(df$lon),mean(df$lat)) # 중요(위도와 경도의 평균으로 센터 주기)
