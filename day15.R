...
webElem <- remDr$findElement(using = "css", "#dateDiv_1 > img") # 달력이미지 선택
webElem$clickElement()
Sys.sleep(1)
webElem <- remDr$findElement(using = "css", "#ui-datepicker-div > table > tbody > tr:nth-child(2) > td:nth-child(6) > a") # 날짜선택
webElem$clickElement()
Sys.sleep(1)
webElem <- remDr$findElement(using = "css", "#cont_body > form > div > div > a:nth-child(1)") # 검색버튼
webElem$clickElement()
Sys.sleep(3)
...
Sys.setlocale() # 다시 원상복구 시키는 기능
Encoding(names(games_table)) <- "UTF-8"  # 제목 인코딩 재설정정
head(games_table)
str(games_table)
View(games_table)
tail(games_table)
...
# (3) 월별 기온 분포 
# 월별 평균기온 계산 :  aggregate(그룹을 나눠서 집계를 내고자 하는 대상, 기준 정보  )
