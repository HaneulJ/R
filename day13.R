...
iris %>% select_if(is.numeric) %>% head(3) # numeric 컬럼에 해당되는 애들만 출력
iris %>% select(-Sepal.Length, -Petal.Length) %>% head(3) # -로 시작하는 애들 출력  
...
# 화면에 출력은 되나 exam에 담지 않아서 exam 변경X
exam %>%
...
iris %>% count(Species) # 변수명에 기반한 행 세기
exam %>% count(class) # 
exam %>% count(id) # 
iris %>% count # 전달된 모든 행 세기
...
exam %>% summarise(n = n())
exam %>% tally() # 이전 식에서 전달한?
exam %>% count()

exam %>% summarise(mean_math = mean(math))  # math 평균 산출
mean(exam$math) 

# 여러가지 요약 통계량을 한번애 출력하고 싶을 때
