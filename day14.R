...
bind_rows(group_a, group_b) # 이게 더 적합 
...
# 두 데이터 프레임을 하나로 join하지만 by에 해당되는 것만!
...
# 5번째 열을 제외한 나머지 추출, Species기준으로 그룹핑, 그룹단위로 평균
...
str(mpg)
ggplot(mpg) # 그래프 그리기 위해서는 무조건 제일 먼저 호출해야 함.
ggplot(mpg,aes(displ, hwy)) # aes: 에스테틱?의 약어(x,y축 정보)
ggplot(mpg,aes(displ, hwy)) + geom_point() # +그리고 싶은 그림의 함수마저 호출(chain 방식)
ggplot(mpg) + geom_point(aes(displ, hwy)) # 이렇게 줘도 됌

# x축 displ, y축 hwy로 지정해 배경 생성
sketch <- ggplot(data = mpg, aes(x = displ, y = hwy))
str(sketch)

sketch + geom_point() # 점 찍기
sketch + geom_line() # 선그래프로 나타내기
sketch + geom_point() + coord_cartesian(xlim=c(3, 6)) # x축 범위 제한
sketch + geom_point() + coord_cartesian(xlim=c(3, 6), ylim=c(10, 30))
sketch + geom_point(aes(color=drv)) # drv: 구동방식 에 따라 색 다르게 표현
...
ggplot(mtcars, aes(x=cyl)) + geom_bar(aes(fill=factor(gear)), alpha=0.5) # alpha: 색깔 투명도
...
ggplot(mtcars, aes(x=gear))+geom_bar() + labs(x="기어수", y="자동차수", title="변속기 기어별 자동차수") + theme_bw() # bw: 블랙 & 화이트
...
ggplot(mtcars, aes(x=gear))+geom_bar() + labs(x="기어수", y="자동차수", title="변속기 기어별 자동차수") + theme_void() # void: 다 없애기
...
library(xlsx)
classDF <- read.xlsx("data/data.xlsx", 1, encoding="UTF-8") # 첫번째 시트만 읽어와라
str(classDF)
View(classDF)

bar_data <- classDF[3] # 열의 인덱스, 데이터 프레임 구조 유지
bar_data
str(bar_data)
table(bar_data$bloodType)
ggplot(bar_data, aes(x=bloodType)) + geom_bar() # bloodType을 기준으로 빈도수 출력
ggplot(classDF, aes(x=bloodType)) + geom_bar(aes(fill=gender)) # gender에 따라서 나눠 출력
...
ggplot(coSalesDF, aes(x=year, y=sales)) + geom_line(size=2, aes(group=company,colour=company))+scale_colour_hue() # hue:기본
ggplot(coSalesDF, aes(x=year, y=sales)) + geom_line(size=2, aes(group=company,colour=company))+scale_colour_manual(values = c("orange", "green")) # manual: 직접 색 지정
...
ggplot(data=w, aes(x=year)) + geom_bar() # x좌표에 적용된 변수의 값을 기준으로 빈도수 
ggplot(data=w, aes(x=year, y=weight)) + geom_bar() # 갯수를 세서 막대를 그리는게 기본이라.. 에러난다!!
ggplot(data=w, aes(x=year, y=weight)) + geom_col()
ggplot(data=w, aes(x=year, y=weight)) + geom_bar(stat="identity") # y축에 대한 변수 지정, _col과 동일
ggplot(data=w, aes(x=year, y=weight)) + geom_bar(stat="identity") + coord_cartesian(ylim=c(60, 75))
ggplot(data=w, aes(x=year, y=weight)) + geom_bar(aes(fill=year), stat="identity") + coord_cartesian(ylim=c(60, 75)) # 연도별로 색깔 다르게
ggplot(data=w, aes(x=year, y=weight)) + geom_bar(aes(fill=year), colour="blue", stat="identity") + coord_cartesian(ylim=c(60, 75))
ggplot(data=w, aes(x=year, y=weight)) + geom_bar(aes(fill=year), stat="identity") + geom_label(aes(label=weight))+coord_cartesian(ylim=c(60, 75)) # label: 데이터값 표현
ggplot(data=w, aes(x=year, y=weight)) + geom_bar(aes(fill=year), stat="identity") + geom_label(aes(label=weight), nudge_y=1)+coord_cartesian(ylim=c(60, 75)) # nudge: 라벨을 1만큼 위로
...
ggplot(mpg, aes(x=displ, y=hwy,  color= manufacturer))+geom_point() + scale_color_brewer(palette = 'Set3') # RColorBrewer를 설치해야 사용가능

# 범례 행 갯수 설정
ggplot(mpg, aes(x=displ, y=hwy,  color= manufacturer))+
  geom_point()+guides(color = guide_legend(nrow = 6)) # 6개 행씩 표시

# 범례 열 갯수 설정
ggplot(mpg, aes(x=displ, y=hwy,  color= manufacturer))+
  geom_point()+ scale_fill_brewer(palette="Reds")+ guides(color = guide_legend(ncol = 2)) # 2열로 표시
...
library(gridExtra)
grid.arrange(p1, p2, p1, p2, nrow = 2) # 2행 2열로 그래프 나타냄(분할 화면)


# 인터랙티브 그래프 만들기
# 패키지 준비하기
install.packages("plotly")
library(plotly)  #ggsave?로 저장가능
# ggplot으로 그래프 만들기
p <- ggplot(data = mpg, aes(x = displ, y = hwy, col = drv)) + geom_point()
p

# 인터랙티브 그래프 만들기, View창에 그려짐 > 이 그래프가 html로 만들어져서 점에 마우스를 대면 
ggplotly(p)
