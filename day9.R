...
remDr <- remoteDriver(remoteServerAddr = "localhost", port = 4445, browserName = "chrome") # selenium에 의해 컨트롤 되는 서버는 크롬
remDr$open()  # open 메서드 호출

remDr$navigate("http://www.google.com/") # 페이지 오픈

webElem <- remDr$findElement(using = "css selector", "[name = 'q']") # 태그에 대한 dom 객체 찾기
webElem$sendKeysToElement(list("PYTHON", key = "enter")) # 검색어 입력하기


remDr$navigate("http://www.naver.com/") # 아규먼트로 주어진 url 호출(자바스크립트로)
str(remDr)

webElem <- remDr$findElement(using = "css selector", "#query") # css selector 대신 xpath도 가능
webElem$sendKeysToElement(list("PYTHON", key = "enter")) # 검색어 입력
str(webElem)
# 브라우저에서의 활동을 프로그래밍에서 자동화로 할 수 있음
...
one$getElementText()
# 에러 안나는 방법~!
remDr$executeScript("arguments[0].click();",list(one)); # Script: 자바스크립트("아규먼트로 자바스크립트 코드")
# 자바스크립트 함수: 가변형인자, 아규먼트
...
more$clickElement() # 클릭이벤트 자동발생, a태그인 '전체댓글'이 눌림
...
for (booksite in booksites) {
  ...
  booktitle <- gsub("[[:cntrl:]]", "", booktitle) # 제어문자 제거
...
remDr$executeScript("scrollTo(0, 0)", args = list(webElem))  # To: 고정된 값으로 이동 
Sys.sleep(1)
remDr$executeScript("scrollBy(0, 3200)", args = list(webElem)) # By: 상대적 거리만큼 이동 ,y:3200 픽셀 내려가라
Sys.sleep(1)
